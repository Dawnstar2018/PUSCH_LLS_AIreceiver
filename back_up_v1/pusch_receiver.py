...
from sionna.phy.ofdm import OFDMDemodulator, LinearDetector,LMMSEInterpolator,LSChannelEstimator
from sionna.phy.utils import insert_dims
from sionna.phy.channel import time_to_ofdm_channel
# from sionna.phy.nr.pusch_channel_estimation import PUSCHLSChannelEstimator
# from sionna.phy.nr.layer_mapping import LayerDemapper
# from sionna.phy.nr.tb_decoder import TBDecoder
...
class PUSCHReceiver(Block):
    ...
    def __init__(self,
        ...
        self.cov_mat_time = cov_mat_time
        self.cov_mat_freq = cov_mat_freq
        self.lmmse_int_time_first = matrix_umi_lmmse
        ...
        if channel_estimator == "ls-lin":
            ...
            self._channel_estimator = sionna.phy.nr.PUSCHLSChannelEstimator(
                                ...
                                precision=self.precision)#['mmse_dp','lin']
        ...
        elif channel_estimator=="mmse_cdl":            
            ...
            self._channel_estimator = sionna.phy.nr.PUSCHLSChannelEstimator(
                                ...
                                pusch_transmitter._dmrs_additional_position,
                                pusch_transmitter._num_cdm_groups_without_data,
                                interpolator=LMMSEInterpolator(self.resource_grid.pilot_pattern,self.cov_mat_time,self.cov_mat_freq),
                                precision=self.precision)
        
        elif channel_estimator == "mmse_umi": 
            # breakpoint()
            self._channel_estimator = sionna.phy.nr.PUSCHLSChannelEstimator(
                                self.resource_grid,
                                pusch_transmitter._dmrs_length,
                                pusch_transmitter._dmrs_additional_position,
                                pusch_transmitter._num_cdm_groups_without_data,
                                interpolator=self.lmmse_int_time_first,
                                precision=self.precision)
        elif channel_estimator == "mmse":
    ...
    def call(self, y, no, h=None):
        ...
        if self._perfect_csi:
            ...
            #y.shape=[1, 1, 16, 14, 192]
            #h.shape=[1, 1, 16, 4, 2, 14, 192]
        # MIMO Detection
        # breakpoint()
